function updateNeutrinoBoostDisplay(){
	...
	if (player.ghostify.neutrinos.boosts >= 3) document.getElementById("neutrinoBoost3").textContent = getFullExpansion(Math.round(tmp.nb[3]*100)/100)
	if (player.ghostify.neutrinos.boosts >= 4) document.getElementById("neutrinoBoost4").textContent = (tmp.nb[4] * 100 - 100).toFixed(1)
	if (player.ghostify.neutrinos.boosts >= 5) document.getElementById("neutrinoBoost5").textContent = getFullExpansion(Math.round(tmp.nb[5]*1000)/10)
	...
	if (player.ghostify.neutrinos.boosts >= 12) document.getElementById("neutrinoBoost12").textContent = "^"+tmp.nb[12].toFixed(4) 
}
...
function updateNeutrinoUpgradeDisplay(){
	...
	if (player.ghostify.hb.unl && tmp.ngp3c) {
		document.getElementById("neutrinoUpg16Pow").textContent=tmp.nu[7].toFixed(3)
		document.getElementById("neutrinoUpg18Pow").textContent=getFullExpansion(tmp.nu[8])
	}
	var sum = player.ghostify.neutrinos.electron.add(player.ghostify.neutrinos.mu).add(player.ghostify.neutrinos.tau).round()
	for (var u = 1; u <= 18; u++) {
		var e = false
		if (u > 15) e = player.ghostify.hb.unl && tmp.ngp3c
...
function updateNeutrinosTab(){
	...
	if (player.ghostify.ghostParticles.gte(Decimal.pow(4,scaleNeutrinoMulti(player.ghostify.neutrinos.multPower)-1).times(2))) document.getElementById("neutrinoMultUpg").className = "gluonupgrade neutrinoupg"
...
function getMaxUnlockedNeutrinoBoosts() {
	...
	if (tmp.ngp3c && player.ghostify.hb.higgs > 0) x++
	return x
}
...
function scaleNeutrinoMulti(x, reversed=false) {
	if (reversed) {
		if (x >= 400) x = Math.sqrt(x*400)
	} else {
		if (x >= 400) x = Math.pow(x, 2)/400
	}
	return x;
}

function buyNeutrinoMult() {
	let cost = Decimal.pow(4, scaleNeutrinoMulti(player.ghostify.neutrinos.multPower) - 1).times(2)
...
function maxNeutrinoMult() {
	let cost = Decimal.pow(4,scaleNeutrinoMulti(player.ghostify.neutrinos.multPower)-1).times(2)
	if (!player.ghostify.ghostParticles.gte(cost)) return
	let toBuy = Math.max(Math.floor(scaleNeutrinoMulti(player.ghostify.ghostParticles.times(3).add(1).log(4), true)-player.ghostify.neutrinos.multPower+1), 1)
...
var neutrinoBoosts = {
	boosts: {
		1: function(nt) {
			...
			if (player.achievements.includes("ng3p96") && tmp.ngp3c) nb1mult *= 1.5
		...
		10: function(nt) {
			...
			let nb10 = Math.max(nb10neutrinos - (tmp.ngp3c?300:3e3), 0) / (tmp.ngp3c?5e4:75e4)
		...
		12: function(nt) {
			...
			if (hasNU(9)) nb12neutrinos += nt[2].add(1).log10()*2
			let nb12 = Math.log2(nb12neutrinos+1)*40+1
			if (tmp.qu.bigRip.active) nb12 = Math.pow(nb12, 1/3);
			return nb12;
		},
	}
}
